...
class DuoLingo_LanguageAcquisition(torch.utils.data.Dataset):
    ...
    def make_score_matrix(self, sub_problem, mode):
        ...
        for i in tqdm(range(len(instances))):
            score_matrix[person_ids_int.index(dataset[i]['user']), dataset[i]['token'],
                         len(dataset[i]['history'])] = dataset[i]['response']
            count_matrix[person_ids_int.index(dataset[i]['user']), dataset[i]['token']] += 1.
...
class IRTSimulation(torch.utils.data.Dataset):
    def __init__(
        ...
        if side_info is not None:
            side_info_feat = true_ability + np.random.normal(scale=0.001, size=true_ability.shape)
...
class AbacusDataset(torch.utils.data.Dataset):
    def __init__(self, train=True, **kwargs):
        ...
        answer_mean = [np.mean(all_answers[i]) for i in range(len(all_answers.keys()))]
        answer_std = [np.std(all_answers[i]) for i in range(len(all_answers.keys()))]
